...
| 팀원 | 에이전트 정의 | 역할 | 출력 |
|------|-------------|------|------|
| schema-architect | `.Codex/agents/schema-architect.md` | DB 스키마 + 마이그레이션 + 인증 | `supabase/migrations/`, `server/lib/` |
| backend-engineer | `.Codex/agents/backend-engineer.md` | API 라우트 + 서비스 + 미들웨어 | `server/routes/`, `server/services/` |
| frontend-engineer | `.Codex/agents/frontend-engineer.md` | 컴포넌트 + 스토어 + 페이지 | `client/src/` |
| ai-designer | `.Codex/agents/ai-designer.md` | AI 시스템 프롬프트 + 대화 프로토콜 | `server/services/aiAgent.js` |
| qa-validator | `.Codex/agents/qa-validator.md` | 통합 테스트 + 경계면 검증 | `_workspace/qa/` |
