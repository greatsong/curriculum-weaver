...
3. `supabase/migrations/00015_curriculum_links.sql` — 3계층 링크 품질 테이블
...
성취기준 데이터 적재 후, 기존 AI 생성 링크 1,768개를 `curriculum_links` 테이블에 마이그레이션합니다:
...
이 스크립트는:
- `server/data/generatedLinks.js`의 AI 생성 링크를 변환
- `source_code < target_code` 정규화 (CHECK 제약 충족)
- 중복 제거 후 500개씩 배치 upsert
- 모든 링크를 `status: 'published'`, `generation_method: 'ai'`로 설정
...
- [ ] 그래프 기본 로드 확인: `GET /api/standards/graph` → published 링크만 표시
- [ ] 전체 링크 필터 확인: `GET /api/standards/graph?status=all` → 모든 상태 포함
- [ ] "AI 제안 포함" 토글 동작 확인 (데이터 관리 페이지)
- [ ] non-published 링크가 점선+투명도로 구분되는지 확인
- [ ] 링크 상세 패널에 integration_theme, lesson_hook 필드 표시 확인
- [ ] 링크 상태 변경 API 확인: `PATCH /api/standards/links/:id/status`
