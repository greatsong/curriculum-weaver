 ...
 *
 * ── 원문 대조 단계 (2026-09-05 추가, scripts/audit/) ──
 *  0a. --official-dir <별책 PDF 폴더> : 별책별 파서(scripts/audit/parsers/bookN.py)로 원문을 구조화
 *      → scripts/audit/data/official/별책N.json (검증기 PASS 필수)
 *  0b. 원문 결점 게이트: scripts/audit/gate.py — 정본을 원문과 전 필드(코드·문장·학교급·학년군·
 *      교과 귀속·영역·해설·적용 고려사항·형식) 대조. data/official이 있으면 항상 실행, 결점 1건이면 FAIL.
 *  0c. --apply-official : scripts/audit/apply_official.py --add-missing 으로 정본을 원문 기준으로 재작성한 뒤
 *      게이트를 다시 돌려 0이어야 계속 진행(정본 파일 변경은 git으로 검토·커밋). --prune-unsourced 동반 가능.
 *
 * ── 검사 항목 (기본 모드) ──
 ...
 *   node scripts/update-standards-pipeline.mjs --official-dir ~/Downloads/260720   # 원문 추출 + 결점 게이트
 *   node scripts/update-standards-pipeline.mjs --apply-official [--prune-unsourced] # 정본 재작성 + 게이트 0 확인
 *   node scripts/update-standards-pipeline.mjs --apply --seed-authority             # 검증된 정본 값으로 DB 교체
 *
 * 새 고시(성취기준 추가·개정)가 나오면: 별책 PDF를 폴더에 넣고 --official-dir → 결점 목록 확인 →
 * --apply-official → git diff 검토·커밋 → --apply --seed-authority → 임베딩·링크·레이아웃 후속 단계.
 *
...
const APPLY_OFFICIAL = args.includes('--apply-official');
const PRUNE_UNSOURCED = args.includes('--prune-unsourced');
const SEED_AUTHORITY = args.includes('--seed-authority'); // --apply 시 seed에 --canonical-authority 전달 (원문 검증된 정본일 때만)
const OFFICIAL_DIR = (() => { const i = args.indexOf('--official-dir'); return i >= 0 ? args[i + 1] : null; })();
const AUDIT_DIR = path.join(__dirname, 'audit');
const OFFICIAL_DATA = path.join(AUDIT_DIR, 'data', 'official');

function runPython(label, scriptRelPath, extraArgs = []) {
  console.log(`\n━━━ ${label}: python3 ${scriptRelPath} ${extraArgs.join(' ')} ━━━`);
  const r = spawnSync('python3', [path.join(ROOT, scriptRelPath), ...extraArgs], { cwd: ROOT, stdio: 'inherit', env: process.env });
  return r.status === 0;
}
...
// ═══ 0. 원문 대조 (scripts/audit) ═══
if (OFFICIAL_DIR) {
  const ok = runPython('0a. 원문 구조화 추출', 'scripts/audit/extract_official.py', ['--pdf-dir', OFFICIAL_DIR, '--out-dir', OFFICIAL_DATA]);
  record('0a. 원문 구조화 추출', ok ? 'PASS' : 'FAIL', ok ? `→ ${path.relative(ROOT, OFFICIAL_DATA)}` : '파서 실패 또는 검증기 FAIL — 위 출력 확인');
  if (!ok) { console.log('\n❌ 원문 추출 실패 — 중단'); process.exit(1); }
}
const hasOfficial = fs.existsSync(OFFICIAL_DATA) && fs.readdirSync(OFFICIAL_DATA).some(f => /^별책\d+\.json$/.test(f));
if (!hasOfficial) {
  record('0b. 원문 결점 게이트', 'SKIP', `${path.relative(ROOT, OFFICIAL_DATA)} 에 별책 JSON 없음 (--official-dir 로 생성)`);
} else {
  if (APPLY_OFFICIAL) {
    const ok = runPython('0c. 정본 재작성(원문 기준)', 'scripts/audit/apply_official.py', ['--add-missing', ...(PRUNE_UNSOURCED ? ['--prune-unsourced'] : [])]);
    record('0c. 정본 재작성', ok ? 'PASS' : 'FAIL', ok ? 'server/data/standards.js 갱신 — git diff로 검토 후 커밋' : '재작성 실패');
    if (!ok) process.exit(1);
  }
  const ok = runPython('0b. 원문 결점 게이트', 'scripts/audit/gate.py', ['--allow-missing-official']);
  record('0b. 원문 결점 게이트', ok ? 'PASS' : 'FAIL', ok ? '결점 0' : `결점 있음 → ${path.relative(ROOT, path.join(AUDIT_DIR, 'data', 'gate_report.json'))} (해소: --apply-official)`);
}
...
{
  const seedArgs = SEED_AUTHORITY ? ['--canonical-authority'] : [];
  const ok = runChild(`C. Supabase 시드 (비파괴 upsert${SEED_AUTHORITY ? ', 정본 권위' : ''})`, 'scripts/seed-standards-from-canonical.mjs', seedArgs);
  if (!ok) {
