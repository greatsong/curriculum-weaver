...
const SECTION_END = new RegExp([
  ...
  '(\\[[^\\]]{1,20}\\]|<[^>]{1,20}>)?\\s*가\\s*\\.\\s*내용\\s*체계', // [과목2] 가. 내용 체계 (다음 과목 표)
...
for (const f of readdirSync(BYEOLCHAEK_DIR).filter((x) => /^2022_개정_교육과정_별책\d+\.json$/.test(x))) {
  ...
  while ((m = hdr.exec(full))) {
    ...
    let body = clean(rest.slice(0, endM ? endM.index + 3 : 3500))
    // 다음 성취기준 목록 침범만 컷(줄머리 "[code] 본문"). 인라인 참조 "성취기준([2국01-01])"은 보존.
    const nextStd = new RegExp('\\n\\s*' + CODE_G.source + '\\s*[가-힣]').exec(body)
    if (nextStd) body = body.slice(0, nextStd.index).trim()
...
// 완전본 (나) 맵을 파일로 저장 (완결성 수정 스크립트가 소비)
writeFileSync(path.join(__dirname, 'results', 'guidance-map.json'), JSON.stringify(Object.fromEntries(guidanceByCode), null, 1))
if (args.includes('--extract-only')) { console.log('extract-only: 맵만 저장, 미적용'); process.exit(0) }
