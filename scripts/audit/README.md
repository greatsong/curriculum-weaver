...
- **코드 충돌(같은 코드, 다른 과목)**: 정본 식별자는 (code, subject) 복합 키. 게이트 `I_코드충돌`은 정보 항목이며, apply가 정본에 없는 과목의 원문 레코드를 `key: "code|subject"`로 추가한다(2026-09-05: 심화 독일어 8·스페인어권 문화 3). 링크 끝점·DB 유일 키·임베딩 키는 이 key를 쓴다.
