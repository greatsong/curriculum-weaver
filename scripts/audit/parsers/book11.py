#!/usr/bin/env python3
"""[별책11] 체육과 교육과정 PDF → 원문 구조화 JSON (SPEC.md 구조).

CLI : python3 book11.py --pdf <PDF경로> [--hwp <HWP경로>] --out <출력JSON>
...
BOOK_NO = 11
MANUAL_NOTES = [
    "해설 불릿 글머리는 Wingdings 사설영역 글리프(U+F09F) — 글머리 기호로 간주해 제거",
    "(나) 고려사항 뒤에 오는 '신체활동 예시' 표는 불릿이 아니라 application_notes에 넣지 않음(영역별 notes 참조)"
]
