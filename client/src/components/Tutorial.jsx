...
const STEPS = [
  ...
  },
  {
    ...
    lines: [
      ...
      { text: '' },
      { text: '성취기준 검색: 교과별, 학년별로 검색해요.', icon: 'search' },
      { text: '   관련 성취기준을 찾아 수업에 연결하세요.' },
      { text: '' },
      { text: '연결 유형: 성취기준 간 관계를 지정해요.', icon: 'link' },
      { text: '   교과 간 융합, 선수 학습, 심화/확장 등' },
      { text: '' },
      { text: '3D 지식 그래프: 성취기준 연결을 시각화해요.', icon: 'graph', isNew: true },
      { text: '   TF-IDF 임베딩 기반 의미적 유사성으로' },
      { text: '   교과 간 관계를 직관적으로 탐색할 수 있어요.' },
      { text: '' },
      { text: '팁: A단계(분석)에서 이 기능을 적극 활용하세요!', icon: 'bulb' },
    ],
  },
  {
    title: '함께 시작해요!',
    lines: [
      ...
      { text: '같은 보드에서 실시간으로 함께 작업해요.' },
      { text: '' },
      { text: '분산인지의 핵심: 한 사람이 모든 걸 알 필요 없어요.', icon: 'team' },
      { text: '   각자의 교과 전문성이 팀의 집단 지성이 됩니다.' },
      { text: '' },
      { text: '설계 흐름 요약:', icon: 'flow' },
      { text: '   T 팀 구성 → A 분석 → Ds 설계' },
      { text: '   → DI 개발·실행 → E 성찰·평가' },
      { text: '' },
