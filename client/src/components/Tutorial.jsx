...
const STEPS = [
  {
    ...
    lines: [
      ...
      { text: '교육과정의 우주에서 별을 잇듯,' },
      { text: '교과를 연결해 수업이라는 별자리를 만들어요.' },
      { text: '' },
  ...
  },
  {
    ...
    badge: '2',
    lines: [
      { text: '작업 공간은 이렇게 구성되어 있어요.', highlight: true },
      { text: '' },
      { text: '상단 — 절차 내비게이션', icon: 'map' },
      { text: '   5개 과정 18개 세부활동을 자유롭게 오가요.' },
      { text: '' },
      { text: '왼쪽 — 설계 보드', icon: 'board' },
      { text: '   AI 제안이 보드 카드로 정리돼요.' },
      { text: '   비전, 주제탐색, 차시구성표 등' },
      { text: '   단계별 설계 산출물이 쌓여가요.' },
      { text: '' },
      { text: '오른쪽 — AI 공동설계자 채팅', icon: 'chat' },
      { text: '   현재 단계에 맞는 설계 대화를 나눠요.' },
      { text: '' },
      { text: '하단 "원칙" 버튼 — 협력 원리 드로어', icon: 'principle' },
      { text: '   현재 세부활동의 협력 원리를 펼쳐 봐요.' },
      { text: '' },
  ...
  },
  {
    ...
    badge: '4',
    lines: [
      { text: '활성화·외현화·조정·상호의존·인지분산, 5가지 협력 원리예요.', highlight: true },
      { text: '' },
      { text: '협력 원리는 AI의 두뇌 역할을 해요.', icon: 'principle' },
      { text: '   AI가 응답할 때 해당 세부활동의 원리를' },
      { text: '   자동으로 참고해서 조언해요.' },
      { text: '' },
      { text: '예시:', icon: 'star' },
      { text: '   "활성화" — 각자 생각·경험을 먼저 꺼내 대화 트기' },
      { text: '   "외현화" — 생각을 키워드·보드로 눈에 보이게' },
      { text: '   "조정" — 흩어진 의견을 묶고 우선순위 맞추기' },
      { text: '' },
      { text: '각 협력 원리에는 가이드라인과 점검 질문이 있어요.', icon: 'check' },
  ...
  },
  {
    ...
    badge: '6',
    lines: [
      { text: '교육과정의 우주에서 별을 잇듯 교과를 연결해요.', highlight: true },
      { text: '' },
      { text: '설계 모드 — 4가지 렌즈로 연결을 탐색해요.', icon: 'search' },
      { text: '   이웃: 내 성취기준이 쓰이는 다른 교과의 장면' },
      { text: '   주제: "기후변화"로 어떤 교과가 연결되는지' },
      { text: '   계열: 앞뒤 학습 흐름 / 과목쌍: 두 교과의 연결' },
      { text: '' },
      { text: '실생활 문제 시나리오: 연결을 고르면 AI가', icon: 'star', isNew: true },
      { text: '   상황·데이터·핵심 질문·활동 흐름까지 초안을 만들어요.' },
      { text: '   맥락 여러 개를 묶어 하나의 시나리오로도 가능!' },
      { text: '' },
      { text: '탐험 3D — 교육과정 성운을 감상해요.', icon: 'graph' },
      { text: '   우주 여행 투어로 교과군을 차례로 둘러보고,' },
      { text: '   주제 스포트라이트로 원하는 주제만 밝혀요.' },
      { text: '' },
      { text: '팁: 마음에 드는 성취기준은 담아두세요 —', icon: 'bulb' },
      { text: '   프로젝트를 만들 때 자동으로 포함돼요!' },
    ],
  },
  {
    title: '함께 시작해요!',
