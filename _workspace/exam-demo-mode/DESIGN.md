...
## 10. v1.1 후보 — 실연 전사 피드백 (타이밍 optional)

> MVP(Stage 0~4) 완료 후 별도 착수. **Stage 4의 채점 루브릭에 의존**(전사를 루브릭 렌즈로 채점)하므로 그 뒤에 얹는다. 여기 기록은 스펙 확정이 아니라 방향·경계 고정용.

### 10.1 개요·동기
지금까지의 시연 모드는 전부 **계획**(과정안·발문·판서·대본·루브릭)이다. 임용 2차에서 실제로 채점받는 건 **말로 하는 실연**이며, 그 공백을 메우는 기능. 사용자가 리허설을 녹음→외부 STT로 전사한 **텍스트를 붙여넣으면**, AI가 계획 보드(과정안·대본·루브릭)를 기준으로 **내용·발문·구성·시간 배분** 피드백을 준다. 도구를 "계획 도구 → 리허설 코치"로 끌어올리는 루프 완성: 계획 → 대본 → 리허설 → **전사 피드백** → 수정.

### 10.2 역할 분담 (범위 경계 — 확정)
- **도구가 하는 것**: 내용 커버리지, 발문 위계(사실→적용→일반화)가 실제로 구현됐는지, 목표–활동–형성평가 정합성, 구성 흐름, **시간 배분(타임스탬프 있을 때)**.
- **도구가 안 하는 것 = 사람(지도교수·스터디)이 맡음**: 발성·속도·시선·태도·판서 시연 등 비언어. → 사용자 확인(2026-07-13): 비언어는 사람 피드백으로 제공 예정이므로 도구는 텍스트가 잘하는 것에만 집중. "과대약속" 리스크 해소.

### 10.3 입력 — 전사 텍스트, 타임스탬프 optional
- 붙여넣기 텍스트 1건(10~15분 발화 ≈ 1,500~2,500단어, 컨텍스트 여유). 오디오 파이프라인/서버 STT 불필요(사용자가 외부에서 전사).
- **타임스탬프는 있을 수도 없을 수도** 있는 것으로 설계. 자동 감지 + 점진적 확장(graceful degradation):
  - **감지**: `[00:32]`, `(1:05:12)`, `00:32` 등 STT 표기를 정규식으로 탐지 → 타이밍 모드 on/off.
  - **있을 때**: 발화를 과정안/대본 구간(도입·전개·정리)에 매핑해 **실제 소요 시간 vs 계획 시간** 비교(구간 초과/미달, 총 10~15분 창 준수, 한 발문 과체류/긴 침묵).
  - **없을 때**: 시간 피드백 **생략**, 구성·발문·내용만. **가드: 타임스탬프 없으면 AI가 시간 피드백을 지어내지 못하게 프롬프트에서 명시 차단**(할루시네이션 방지).
- 타이밍은 텍스트가 유독 잘하는 영역(발성과 달리 영상 없이도 정확) → 임용의 가장 딱딱한 제약(10~15분 엄수·초과 감점)을 도구의 강점으로 전환.

### 10.4 데이터 모델
- 신규 보드 `demo_rehearsal`(BOARD_TYPES/SCHEMAS **추가만**): `transcript`(textarea, 붙여넣기)·`hasTimestamps`(자동판정, 표시용)·`feedback`(ai_suggestion로 채우는 구조화 피드백: 내용/발문/정합성/시간 섹션)·(선택)`timingTable`(구간별 계획 vs 실제). 신규 DB migration 0건(기존 designs.content jsonb).
- DemoStepNav에 스텝 추가: ①성취기준 ②교수학습과정안 ③실연 대본 ④**실연 피드백**.

### 10.5 AI 처리
- "실연 피드백" 트리거 → `buildSystemPrompt({mode:'demo', tone:'coaching'})`에 전사 + 과정안·대본·**루브릭(Stage 4)** 보드를 컨텍스트로 주입 → 루브릭을 **채점 렌즈**로 재사용한 구조화 피드백을 `<ai_suggestion procedure="demo_rehearsal">`로.
- **인용 근거 필수**: 피드백은 전사에서 실제 문장을 인용해 뒷받침(시나리오 작위성 방지 패턴 재사용). 전사에 없는 것을 지어내지 않음.
- coherence 매핑 재사용: "계획한 발문 vs 실제 사용한 발문" diff.

### 10.6 재사용 vs 신규
- **재사용**: materialAnalyzer 텍스트 인입 패턴, AI 에이전트(demo 분기·코치/채점관 톤), 루브릭(Stage 4), coherence, 보드+ai_suggestion+수락 경로, DemoGenerateToolbar 트리거 패턴.
- **신규**: demo_rehearsal 보드·스키마, 타임스탬프 파서 + 구간 매핑, 타이밍 비교 UI, 전사 인입 UI.

### 10.7 리스크·완화
- STT 노이즈로 축자 비교는 부정확 → 구조·흐름 위주 피드백(품질 상한 인정).
- 타이밍 매핑 오정렬(발화↔구간) → 휴리스틱 매핑 + 불확실하면 "구간 매핑 불확실" 표기, 강제 점수화 금지.
- 배치: **Stage 4 완료 후** 착수(루브릭 의존). 현재 진행 중 5단계 스코프에 끼워넣지 않음.
